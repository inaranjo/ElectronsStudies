...
//   TString pathToFile = "/data_CMS/cms/ivo/AntiEMVANewTraining/Trees/AntiEMVATrees-DYJetsToLL-madgraph-DR53X-Summer12-iter2/";
  ...
  std::vector<std::string> tauDecays ; 
  tauDecays.push_back("All");
//   tauDecays.push_back("OneProng");
//   tauDecays.push_back("TwoProngs");
//   tauDecays.push_back("ThreeProngs");


  for ( std::vector<std::string>::const_iterator matching = matchings.begin();
	matching  != matchings.end(); ++matching ) {

    for ( std::vector<std::string>::const_iterator tauDecay = tauDecays.begin();
	  tauDecay  != tauDecays.end(); ++tauDecay ) {
	...
	TauAbsEtaSelection = "(Tau_EtaAtEcalEntrance>1.479 && Tau_EtaAtEcalEntrance<3.0) || (Tau_EtaAtEcalEntrance>-3.0 && Tau_EtaAtEcalEntrance<-1.479)";
      ...
      TCut TauDecaySelection = "";
      TCut CategorySelection = "";
      TCut IsolationSelection = "";

      if(tauDecay->find("One")!=std::string::npos)    TauDecaySelection = "Tau_DecayMode>-1 && Tau_DecayMode<5";
      else if(tauDecay->find("Two")!=std::string::npos)   TauDecaySelection = "Tau_DecayMode>4 && Tau_DecayMode<10";
      else if(tauDecay->find("Three")!=std::string::npos) TauDecaySelection = "Tau_DecayMode>9 && Tau_DecayMode<15";
	...
	if (category == "All") CategorySelection = "Tau_MatchElePassVeto<0.5"; 
      ...
      TCut ElecSelection = TauDecaySelection && CategorySelection && IsolationSelection && PUSelection && ElecPtSelection && ElecAbsEtaSelection && ElecMatchSelection ;
      TCut TauSelection = TauDecaySelection && CategorySelection && IsolationSelection && PUSelection && TauPtSelection && TauAbsEtaSelection && TauMatchSelection ;
      TCut Selection;
      if (variable.find("Elec")!=std::string::npos)Selection = ElecSelection;
      if (variable.find("Tau")!=std::string::npos)Selection = TauSelection;
    

      TH1F* hVariable = new TH1F(Form("hVariable_%s",tauDecay->data()),"" , nBins ,xMin, xMax);
...
//       if (tauDecay->find("One")!=std::string::npos){
// 	//     hVariable->SetFillColor(kRed);
// 	//     hVariable->SetFillStyle(3345);
// 	hVariable->SetLineColor(kRed);
// 	hVariable->SetLineWidth(2);
//       }
//       else if (tauDecay->find("Two")!=std::string::npos){
// 	//     hVariable->SetFillColor(kRed);
// 	//     hVariable->SetFillStyle(3345);
// 	hVariable->SetLineColor(kBlue);
// 	hVariable->SetLineWidth(2);
//       } 
//       else if (tauDecay->find("Three")!=std::string::npos){
// 	//     hVariable->SetFillColor(kRed);
// 	//     hVariable->SetFillStyle(3345);
...
//       cout<<"  Total number of Candidates : "<<hVariable->GetEntries()<<endl;
      ...
      cout<<"Tau decay : "<<tauDecay->data()<<endl;
      cout<<"Selection : "<<Selection<<endl;
      inputTree->Draw(Form("%s>>hVariable_%s",variable.data(),tauDecay->data()),Selection);
...
//       leg->AddEntry(hVariable,Form("%s",tauDecay->data()));
    ...
    else if(xAxisTitle == "E_{#gamma}/(P_{in}-P_{out})" ){
      refHistogram->SetMaximum(0.03);
      refHistogram->SetMinimum(0.0);
    }

    else if(xAxisTitle == "HadrMva(#tau)" ){
...
//     string outputName = Form("plots/plotVariablesAntiEMVA/v14/%s/plotVariablesAntiEMVA_v14%s_%s_%s_%s",category.Data(),discriminator.data(),category.Data(),variable.data(),Region.Data());
//   string outputName = Form("plots/plotVariablesAntiEMVA/v14/ByDecayModes/plotVariablesAntiEMVA_v14%s_%s_%s_%s_%s",discriminator.data(),category.Data(),variable.data(),Region.Data(),matching->data());
    string outputName = Form("plots/plotVariablesAntiEMVA/v18/%s/plotVariablesAntiEMVA-v18%s_%s_%s_%s",category.Data(),discriminator.data(),category.Data(),variable.data(),Region.Data());
...
//   variables.push_back(std::string("Tau_DecayMode"));
...
//   variables.push_back(std::string("Tau_GsfEleMatch"));
  ...
  xAxisTitles["Tau_DecayMode"]     = "Decay Mode(#tau)" ;
