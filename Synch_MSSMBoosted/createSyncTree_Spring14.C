...
void synchNtuple(string sample = "GGFH125", string stream = "MuTau", TString path="", TString file="", bool incl=false) {
  ...
  TFile *lOFile = new TFile(("Synch_"+sample+"_"+stream+".root").c_str(),"RECREATE");
...
//    float lagainstMuonTight2         = 0; lOTree->Branch("againstMuonTight2_2"       ,&lagainstMuonTight2           ,"lagainstMuonTight2/F"     );
   ...
   int iHLTmatch      ; tree->SetBranchAddress("HLTmatch"    ,&iHLTmatch);
   float iHLTmatchMu      ; if(sample.find("Luca") != string::npos && stream == "MuTau")tree->SetBranchAddress("HLTmatchMu"    ,&iHLTmatchMu);
   float iHLTmatchTau      ;if(sample.find("Luca") != string::npos && stream == "MuTau")tree->SetBranchAddress("HLTmatchTau"    ,&iHLTmatchTau);
...
//    float iIso2        ; tree->SetBranchAddress("hpsMVA"      ,&iIso2           );//MVA iso for hadronic Tau, Delta Beta for muon
   ...
   float ihpsMVA3oldDMwLT ; tree->SetBranchAddress("hpsMVA3oldDMwLT"       ,&ihpsMVA3oldDMwLT          );
//    float ihpsMVA2         ; tree->SetBranchAddress("hpsMVA2"       ,&ihpsMVA2            );
   ...
   if(sample.find("Luca") != string::npos && stream == "MuTau")     selection= selection && "HLTx && HLTmatchMu && HLTmatchTau" ;//Luca's check
   else selection = selection && "HLTx && HLTmatch>0" ;//Ivo's version
//    else selection = selection && "HLTx " ;//Ivo's version
   ...
   cout<<"tree in file entries : "<<tree->GetEntries()<<endl;
   tree->Draw(">>+skim", selection ,"entrylist");
//    tree->Draw(">>+skim", "" ,"entrylist");
...
//      if(iEvt!=617988440 )continue;
//      if(iEvt!=52036512 )continue;

//
//      if(iRun!=203830 || iLumi!=108 || iEvt!=55233994) continue ;//Selected by Ivo but not by Luca
//      if(iRun!=204566 || iLumi!=8 || iEvt!=5727020) continue ;//Selected by Luca but not by Ivo

//      if(iRun!=206245 || iLumi!=56 || iEvt!=62430802) continue ;//Selected by Luca but not by Ivo

// //Selected by Ivo but not by Luca
//      if(!(iRun==206187 && iLumi==157 && iEvt==165270716)&&//Selected by Ivo but not by Luca
// 	!(iRun==206243 && iLumi==770 && iEvt==1079688543)&& //Selected by Ivo but not by Luca
// 	!(iRun==206243 && iLumi==787 && iEvt==1098286255)&& //Selected by Ivo but not by Luca
// 	!(iRun==206245 && iLumi==11 && iEvt==12576911)&& //Selected by Ivo but not by Luca
// 	!(iRun==206245 && iLumi==33 && iEvt==36839450)&& //Selected by Ivo but not by Luca
// 	!(iRun==206246 && iLumi==682 && iEvt==630635876)&& //Selected by Ivo but not by Luca
// 	!(iRun==206246 && iLumi==722 && iEvt==662138902)&& //Selected by Ivo but not by Luca
// 	!(iRun==206243 && iLumi==548 && iEvt==820864187)&& //Selected by Luca but not by Ivo
// 	!(iRun==206243 && iLumi==827 && iEvt==1140750009)&& //Selected by Luca but not by Ivo
// 	!(iRun==206243 && iLumi==830 && iEvt==1144007857)&&//Selected by Luca but not by Ivo
// 	!(iRun==206245 && iLumi==56 && iEvt==62430802)
// 	) continue ;//Selected by Luca but not by Ivo
...
//      if(sample.find("Luca"))cout<<"Luca :"<<endl;
//      else cout<<"Ivo :"<<endl;
//      std::cout<<"Event     HLTx     HLTmatch  Pt1     Pt2     scEta1     Eta1     Eta2    ElecMVA    VetoFlag    eIso    tauIso   3rdLepVeto     Mt   ihpsMVA3oldDMwLT "<<std::endl;
//      cout<<iEvt<<"  "<<iHLTx<<"           "<<iHLTmatch<<"    "<<iPt1<<"  "<<iPt2<<"  "<<iscEta1<<"  "<<iEta1<<"  "<<iEta2<<"  "<<iPassId1T_F<<"    "<<iMuFlag<<"      "<<iIso1<<"  "<<ihpsDB3H<<"    "<<ivetoEvent<<"            "<<iMt1<<"      "<<ihpsMVA3oldDMwLT<<endl;
          
     //      cout<<iEvt<<"  "<<iPt2<<"  "<<iEta2<<"  "<<endl; 
...
//   TString path="/data_CMS/cms/htautau/PostMoriond/NTUPLES_NewTauIDVariables/MuTau/";
//   TString pathLuca="/data_CMS/cms/htautau/PostMoriond/NTUPLES_NewTauIDVariables/MuTau/LucaCheck/";
//   TString path2="/data_CMS/cms/htautau/PostMoriond/NTUPLES_NewTauIDVariables/EleTau/";
//   TString path2="/data_CMS/cms/htautau/PostMoriond/NTUPLES_NewTauIDVariables/EleTau/temp/";
//   TString path2Luca="/data_CMS/cms/htautau/PostMoriond/EleTau/temp/";
//   TString path2Luca="/data_CMS/cms/htautau/PostMoriond/NTUPLES_NewTauIDVariables/EleTau/temp_check/";

  TString pathEleTau="/data_CMS/cms/htautau/PostMoriond/NTUPLES_OlivierTES/EleTau/";
  TString pathMuTau="/data_CMS/cms/htautau/PostMoriond/NTUPLES_OlivierTES/MuTau/";
...
//   synchNtuple("DataRunD-p1", "MuTau",path,"nTupleRun2012D-22Jan2013-p1-Data_MuTau.root", true);
//   synchNtuple("DataRunD-p1_Luca", "MuTau",pathLuca,"nTupleRun2012D-22Jan2013-p1-Data_MuTau.root", true);
//   synchNtuple("DataRunD-p4_Luca", "MuTau",pathLuca,"nTupleRun2012D-22Jan2013-p4-Data_MuTau.root", true);
//   synchNtuple("DataRunD-p4", "MuTau",path,"nTupleRun2012D-22Jan2013-p4-Data_MuTau.root", true);
//   synchNtuple("DataRunD_Luca", "MuTau",pathLuca,"nTupleRun2012D-22Jan2013-p*-Data_MuTau.root", true);
//   synchNtuple("DataRunD", "MuTau",path,"nTupleRun2012D-22Jan2013-p*-Data_MuTau.root", true);

//   synchNtuple("DataRunD-p4", "ElecTau",path2,"nTupleRun2012D-22Jan2013-p4-Data_ElecTau.root", true);
//   synchNtuple("DataRunD-p4_Luca", "ElecTau",path2Luca,"nTupleRun2012D-22Jan2013-p4-Data_ElecTau.root", true);
//   synchNtuple("DataRunD-p4", "ElecTau",path2,"nTupleRun2012D-22Jan2013-p4-Data_ElecTau.root", true);
//   synchNtuple("DataRunD-p4_Luca", "ElecTau",path2Luca,"nTupleRun2012D-22Jan2013-p4-Data_ElecTau.root", true);

//Synch with Shalhout
  synchNtuple("SUSYGGH120", "ElecTau",pathEleTau,"nTupleSUSYGGH120_ElecTau_nominal.root", true);
  synchNtuple("SUSYBBH120", "ElecTau",pathEleTau,"nTupleSUSYBBH120_ElecTau_nominal.root", true);
  synchNtuple("SUSYGGH120", "MuTau",pathMuTau,"nTupleSUSYGGH120_MuTau_nominal.root", true);
  synchNtuple("SUSYBBH120", "MuTau",pathMuTau,"nTupleSUSYBBH120_MuTau_nominal.root", true);
  synchNtuple("SUSYGGH300", "ElecTau",pathEleTau,"nTupleSUSYGGH300_ElecTau_nominal.root", true);
  synchNtuple("SUSYBBH300", "ElecTau",pathEleTau,"nTupleSUSYBBH300_ElecTau_nominal.root", true);
  synchNtuple("SUSYGGH300", "MuTau",pathMuTau,"nTupleSUSYGGH300_MuTau_nominal.root", true);
  synchNtuple("SUSYBBH300", "MuTau",pathMuTau,"nTupleSUSYBBH300_MuTau_nominal.root", true);
}
